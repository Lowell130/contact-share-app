...
<script setup>
/**
 * Tipi supportati (aggiunti: strava).
 */
const FIELD_TYPES = [
...
const form = reactive({
  ...
  bio:  props.modelValue.bio  || props.modelValue.notes || '', // compat: se arriva "notes"
...
/** Aggiunge https:// ai link/social se mancante */
...
const onSubmit = () => {
  const cleaned = {
    ...
    slug: (form.slug || '').trim(),
    // salviamo la bio come "notes" per compatibilità backend
    notes: (form.bio || '').trim(),

<template>
  <section class="bg-white dark:bg-gray-900">
    <div class="flex items-center justify-center min-h-screen py-6">
      <div class="bg-white p-6 rounded-lg shadow-lg dark:bg-gray-900">
        <div class="w-full sm:w-[500px] p-4 bg-white sm:p-6 dark:bg-gray-800 mx-auto rounded-lg">
          ...
          <div
            v-if="card.notes"
            class="mb-6 mt-4 border border-amber-300 p-4 rounded-lg bg-amber-50 text-sm text-gray-800 dark:bg-amber-900/40 dark:border-amber-600 dark:text-amber-50"
          >
            {{ card.notes }}
          </div>
          ...
          <ul class="my-6 space-y-3 text-left w-full max-w-xl mx-auto">
            <li v-for="(f, i) in socialFields" :key="i">
              <a
                ...
                :class="[
                  'flex items-center p-3 text-base font-medium rounded-lg group hover:shadow transition',
                  socialClassFor(f)
                ]"
              >
                <!-- Icona -->
...
function firstField(fields, types) {
  ...
  return (fields || []).find(x => t.includes(String(x.type || '').toLowerCase()) && x.value)
}
...
/* Info principali sotto il titolo */
...
const SOCIAL_TYPES = [
  ...
  'tiktok','twitch','strava','telegram','whatsapp','onlyfans','dribbble'
]

const socialFields = computed(() =>
  (props.visibleFields || []).filter(f =>
    SOCIAL_TYPES.includes(String(f.type || '').toLowerCase()) && f.value
  )
)
...
/* Classi colore per social */
function socialClassFor(f) {
  const t = (f?.type || '').toLowerCase()
  const base =
    'text-white hover:opacity-95 dark:text-white'

  if (t === 'facebook') return `bg-[#1877F2] ${base}`
  if (t === 'instagram') return `bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 ${base}`
  if (t === 'linkedin') return `bg-[#0A66C2] ${base}`
  if (t === 'x' || t === 'twitter') return `bg-black ${base}`
  if (t === 'github') return `bg-gray-900 ${base}`
  if (t === 'youtube') return `bg-[#FF0000] ${base}`
  if (t === 'tiktok') return `bg-black ${base}`
  if (t === 'twitch') return `bg-[#9146FF] ${base}`
  if (t === 'strava') return `bg-[#FC4C02] ${base}`
  if (t === 'telegram') return `bg-[#229ED9] ${base}`
  if (t === 'whatsapp') return `bg-[#25D366] ${base}`
  if (t === 'onlyfans') return `bg-[#00AFF0] ${base}`
  if (t === 'dribbble') return `bg-[#ea4c89] ${base}`
...
function iconFor(f) {
  ...
  if (t === 'dribbble') return svgDribbble()
  return svgInfo()
}
...
<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"
     xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  <path fill-rule="evenodd" d="M12 4a8 8 0 0 0-6.895 12.06l.569.718-.697 2.359 2.32-.648.379.243A8 8 0 1 0 12 4ZM2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10a9.96 9.63 0 0 1-5.016-1.347l-4.948 1.382 1.426-4.829-.006-.007-.033-.055A9.958 9.958 0 0 1 2 12Z" clip-rule="evenodd"/>
